...
<c:if test="${ count > 0 }">	
	...
	<tr>
		...
		<td colspan="2"><center>카라멜 가격 </center></td><td><center>${list.cf_caramel }</center></td>

	<tr>
		<td colspan="2"><center>추천수 : ${list.cf_recom }  <c:if test="${userId ne null }">
<input type ="button" value ="추천" onclick="document.location.href='CaramelRecom.do?cf_num=${list.cf_num }&pageNum=${ pageNum }'">
</c:if></center></td>
