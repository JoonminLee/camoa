...
<c:if test="${ count > 0 }">	
	...
	<tr>
		...
		<td colspan="2"><center>라떼 가격 </center></td><td><center>${list.cf_latte }</center></td>

	<tr>
		<td colspan="2"><center>추천수 : ${list.cf_recom }  <c:if test="${userId ne null }">
<input type ="button" value ="추천" onclick="document.location.href='LatteRecom.do?cf_num=${list.cf_num }&pageNum=${ pageNum }'">
</c:if></center></td>
