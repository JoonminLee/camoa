...
<c:if test="${ count > 0 }">	
	...
	<tr>
		<td colspan="3" rowspan="3"><a href="content.do?cf_num=${list.cf_num}&pageNum=${ currentPage }"><center><img
							src="./image/noimage2.png" /></center></a></td>
		<td colspan="2" rowspan="2"><center><h2>${list.cf_name }</h2></center></td>
	</tr>
	<tr>
	</tr>
	<tr>
		<td><center>추천수  ${list.cf_recom } <c:if test="${userId ne null }">
<input type ="button" value ="추천" onclick="document.location.href='ReadcountRecom.do?cf_num=${list.cf_num }&pageNum=${ pageNum }'">
</c:if></center></td>
