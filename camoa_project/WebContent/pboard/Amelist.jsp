...
<c:if test="${ count > 0 }">	
	...
	<tr>
		...
		<td colspan="2"><center>아메리카노 가격 </center></td><td><center>${list.cf_ame }</center></td>

	<tr>
		<td colspan="2"><center>추천수 : ${list.cf_recom }  <c:if test="${userId ne null }">
<input type ="button" value ="추천" onclick="document.location.href='AmeRecom.do?cf_num=${list.cf_num }&pageNum=${ pageNum }'">
</c:if></center></td>
